# -*- coding: utf-8 -*-
"""
 200 hPa Meridional Wind (V) ANOMALY  (ECMWF AIFS forecast − NCEP 1991-2020 climo)
...
def main():
    ...
    print("=" * 64)
    print(f"  AIFS  {LEVEL_HPA} hPa Meridional Wind (V) Anomaly")
    print("=" * 64)
    ...
    print("\n[2/4] AIFS forecast V …")
    lat, lon, fc = C.fetch_aifs_fields(base, steps, [("v", LEVEL_HPA)], CACHE_DIR)

    print("\n[3/4] NCEP climatology vwnd …")
    latc, lonc, cl = C.fetch_ltm_fields([v.date() for v in valid],
                                        [("vwnd", LEVEL_HPA)], CACHE_DIR)

    v_fc = fc[("v", LEVEL_HPA)]
    v_cl = C.interp_to_grid(cl[("vwnd", LEVEL_HPA)], latc, lonc, lat, lon)
    ...
    anom = C.smooth2d(v_fc - v_cl, SMOOTH_DEG / abs(lon[1] - lon[0]))
    ...
    C.draw_anomaly(lat, lon, anom, coast,
                   meta=dict(title=f"{LEVEL_HPA} hPa Meridional Wind (V) Anomaly",
                             period=period,
                             run_txt=f"Run: {base:%HZ} • {base:%-d %b %Y}"),
                   cb_label=f"V{LEVEL_HPA} Anomaly (m/s)",
